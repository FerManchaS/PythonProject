...
def register():
    ...
    if reg_form.validate_on_submit():
        if not User.query.filter_by(email=reg_form.email.data).first():
            if reg_form.password.data != reg_form.password_repeat.data:
                flash('Please input the same password')
                return redirect(url_for('register'))
            else:
                secure_pass = generate_password_hash(reg_form.password.data, 'pbkdf2:sha256', 8)
                new_user = User(
            ...
            flash('User already register, please log in')
